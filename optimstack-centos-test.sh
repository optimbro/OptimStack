...
case $OPTION in
	1)
		...
		yum -o Acquire::ForceIPv4=true update
		echo "System updated"
		sleep 1
		echo "Installing Dependencies..."
		yum -o Acquire::ForceIPv4=true install -y build-essential mailutils whois cmake ca-certificates wget curl libpcre3 libpcre3-dev autoconf unzip automake libtool tar git libssl-dev zlib1g-dev uuid-dev lsb-release libxml2-dev libxslt1-dev
        	yum -o Acquire::ForceIPv4=true install -y apt-utils autoconf automake build-essential git libcurl4-openssl-dev libgeoip-dev liblmdb-dev libpcre++-dev libtool libxml2-dev libyajl-dev pkgconf wget zlib1g-dev git
		yum -o Acquire::ForceIPv4=true install -y libtool autoconf build-essential libpcre3-dev zlib1g-dev libssl-dev libxml2-dev libgeoip-dev liblmdb-dev libyajl-dev libcurl4-openssl-dev libpcre++-dev pkgconf libxslt1-dev libgd-dev
	...
	3) # # Update the script
		...
		sleep 1
		rm -rf optimstack-centos-test.sh
		echo "Starting update..."
		sleep 1
		wget https://raw.githubusercontent.com/OptimBro/OptimStack/master/optimstack-centos-test.sh --no-dns-cache -O optimstack-centos-test.sh
		chmod +x optimstack-centos-test.sh
		echo ""
		echo "Updating script..."
		sleep 3
		./optimstack-centos-test.sh
