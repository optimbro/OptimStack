...
case $OPTION in
	1)
		...
		apt-get -o Acquire::ForceIPv4=true install -y build-essential mailutils whois cmake ca-certificates wget curl libpcre3 libpcre3-dev autoconf unzip automake libtool tar git libssl-dev zlib1g-dev uuid-dev lsb-release libxml2-dev libxslt1-dev
	...
	3) # # Update the script
		...
		sleep 1
		rm -rf optimstack-virtualmin.sh
		echo "Starting update..."
		sleep 1
		wget https://raw.githubusercontent.com/OptimBro/optimstack/master/optimstack-virtualmin.sh --no-dns-cache -O optimstack-virtualmin.sh
		chmod +x optimstack-virtualmin.sh
		echo ""
		echo "Updating script..."
		sleep 3
		./optimstack-virtualmin.sh
